...
- 三条都做了**变异验证**：故意把被测代码改坏（去掉 `never` 分支、调换优先级、放宽字节校验、漏判 MP4 文件头），确认测试会红。补测试时这一步不能省——新写的测试本来就该过，过了不代表验到了点上。
- `audit-untested-enums.mjs` 自查：缺覆盖从 51 项降到 48 项，三条确已离开缺口列表。
