...
- 最多登记 16 个接手会话，新参数只能用于恢复，创建运行时传它会被拒绝。本次只改当前会话接管的 cm-ai 主任务运行；旧 protected 保护模式兼容入口、batch 多任务批次和 QA-fix 修复子任务保持原行为。
...
- 两条都做了**变异验证**：故意把被测代码改坏（去掉 `never` 分支、调换优先级、放宽字节校验、漏判 MP4 文件头），确认测试会红。补测试时这一步不能省——新写的测试本来就该过，过了不代表验到了点上。
- `audit-untested-enums.mjs` 自查：缺覆盖从 51 项降到 49 项，两条确已离开缺口列表。
