...
- 四条都做了**变异验证**：故意把被测代码改坏（去掉 `never` 分支、调换优先级、放宽字节校验、漏判 MP4 文件头；本次让 command 重做、禁止 evaluation 或 snapshot 重做、漏验回执请求摘要），确认测试会红。补测试时这一步不能省——新写的测试本来就该过，过了不代表验到了点上。
- `audit-untested-enums.mjs` 自查：加上上文 cm-fix 升级出口的补测，缺覆盖累计从 51 项降到 45 项、分布在 42 处；本次从 47 项、43 处降下，`snapshot,evaluation` 已离开缺口列表。
