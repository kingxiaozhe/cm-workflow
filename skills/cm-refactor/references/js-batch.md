...
首次准备漏检只修声明测试，最多三版；不能修生产行为使测试通过、换掉原变异或引入依赖。
第二轮补判官及旧审查追加登记见 [判官修订](js-host.md#第二轮判官修订)，只接受一次有界提案。
批量修订也先恢复启动时全部业务文件（含新增前不存在、删除前存在的文件）来验证新版判官；试点回退只恢复业务稿，批次日志继续追加。
