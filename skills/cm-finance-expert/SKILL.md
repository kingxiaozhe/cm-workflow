...
2. **法域未确认前**，一切合规判断必须标注 `[待法域确认]`；法域以 `rules/finance.md` 头部字段为准——有值按值判断，无值一律标注。多仓库时以任一已确认值为准，多值冲突 → 上报
...
## 4. rules/finance.md 模板（供 /cm:init 或 bootstrap 生成，本 skill 不执行写入）

```markdown
---
description: 金融项目开发铁律
法域: 待确认        # 人工确认后填写，如 "SG,US"；本字段是合规判断的前提
---

# 金融开发铁律

- 金额一律 decimal/整数最小单位，禁止 float；精度与舍入规则全局统一并显式声明
- 所有资金相关写操作必须幂等（幂等键），必须留审计日志（谁/何时/何值/前值）
- 资金流向可追溯：任何余额变动都能回溯到触发事件
- 费率/汇率/价格有明确的取值时点和来源，禁止隐式取"当前值"
- 涉及用户资产展示的数字必须标注计算口径和更新时间
```
