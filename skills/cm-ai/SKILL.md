...
用户明确要求 JS workflow，或恢复已有 JS host/batch 运行时，先读取
`references/js-host.md`，使用共享 JS 入口驱动阶段；当前会话只执行它请求的工具工作。
这条路由不执行下文旧流程中的手工状态、日志、handoff、Review 凭证与任务勾选步骤，
这些写入仍由 JS 的原 owner 和唯一完成门禁负责。节点参考中的业务约束仍适用。
不支持的宿主、目录或业务阶段必须明确阻断，不能静默切回手工流程或另建完成路径。

这是源 Skill 的显式接入，不代表已安装版本已更新或 Codex/Claude 全阶段已验收。
未选择 JS 且没有已有 JS 运行时，继续以下兼容流程；不能把兼容流程计作 JS 改造完成。
