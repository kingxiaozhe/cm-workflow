...
- **审查凭证落盘**：原始结果写入 `{SPECS_DIR}/.reviews/fix-{slug}-r{轮次}.md`；进入第 6 步前先运行 `ls {SPECS_DIR}/.reviews/fix-{slug}-r*.md` 定位凭证，无输出则待审；再实际读取，核对本次范围、内容、轮次和真实 fresh 通道，须为 `independent: true` 且 `verdict: approved`。文件存在只是必要条件，自审 approved 或旧内容批准均不能放行；文件名写入第 7 步输出
