...
import {
  GateError,checkN4,checkN5,loadHandoff,prepareMarkDone,verifyMarkDonePlan,
  checkParallelWrite,
...
function writeHandoff(file,{attempt=1,status='ready_for_review',task='T-001'}={}){
  const payload={
    schema_version:1,
    task_id:task,
    attempt,
...
test('JS read-only task gate handles the full attempt-2 N4/N5 chain',()=>fixture(root=>{
  ...
  assert.equal(checkN4({handoff:handoff2,reviewsDir:reviews,feature:'login',task:'T-001'}).outcome,'ready_for_review');
  assert.equal(checkN5({handoff:handoff2,reviewsDir:reviews,feature:'login',task:'T-001'}).outcome,'approved');

  const args=['check-n5','--handoff',handoff2,'--reviews-dir',reviews,'--feature','login','--task','T-001'];
  ...
  const selectors={handoff:handoff2,reviewsDir:reviews,feature:'login',task:'T-001',tasksPath:tasks};
...
test('JS read-only task gate rejects malformed handoff and non-approved N5 evidence',()=>fixture(root=>{
  ...
  assert.throws(()=>checkN5({handoff:duplicate,reviewsDir:reviews,feature:'login',task:'T-001'}),GateError);
}));
