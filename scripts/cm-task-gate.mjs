...
function loadSchemaContract(){
  ...
  try{
    ...
    if(fields.size!==Object.keys(properties).length||Object.keys(properties).some(key=>!fields.has(key)))
      throw new Error('required/properties mismatch');
    return {
...
export function loadHandoff(handoff,{task=null,attempt=null}={}){
  ...
  const missing=[...SCHEMA.handoffFields].filter(field=>!fields.has(field)).sort();
  const unknown=[...fields].filter(field=>!SCHEMA.handoffFields.has(field)).sort();
...
export function checkN4({handoff,reviewsDir,feature,task}){
  ...
  return {gate:'n4',task,attempt,outcome:'ready_for_review',handoff_sha256:sha256(handoff)};
}

export function checkN5({handoff,reviewsDir,feature,task}){
  requireTaskId(task,'task');
  const payload=loadHandoff(handoff,{task});
  ...
  return {gate:'n5',task,attempt,outcome:'approved',review:fs.realpathSync(review)};
}
...
function pinApproval({handoff,reviewsDir,feature,task}){
  ...
  const approval={...checkN5({handoff,reviewsDir,feature,task})};
  requireRevisions(revisions);
...
function usage(){return 'usage: cm-task-gate.mjs {validate-handoff|check-n4|check-n5|prepare-mark-done|verify-mark-done-plan|check-parallel-write} [options]';}

function parseCli(argv){
  if(argv.length===0||argv.includes('--help')||argv.includes('-h'))return {help:true};
  const command=argv[0];
  const allowed=command==='validate-handoff'
    ?new Set(['--handoff','--task','--attempt'])
  ...
  if(!['validate-handoff','check-n4','check-n5','prepare-mark-done','verify-mark-done-plan','mark-done-locked','check-parallel-write'].includes(command))
    throw new GateError('unsupported command');
  const values={command};
  for(let index=1;index<argv.length;index+=2){
    const flag=argv[index],value=argv[index+1];
    if(!allowed.has(flag)||value===undefined)throw new GateError('invalid arguments');
    const key={'--handoff':'handoff','--reviews-dir':'reviewsDir','--feature':'feature','--task':'task','--attempt':'attempt',
      '--tasks':'tasksPath','--expected-plan-digest':'expectedPlanDigest','--repo':'repo','--assignment':'assignment'}[flag];
    if(key==='assignment'){
