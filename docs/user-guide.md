...
`cm-check` 默认先查 npm 稳定版，有新版就升级已管理的插件，再检查升级后的目录，
不需要另加参数或再次确认。升级后其他 Skills 需新开会话加载。离线时继续本地检查，
但会明确提示“无法确认最新版”。源码仓库、Pi/BYZ 等其他管理器安装保留原方式，
自动升级当前支持 macOS Codex 个人本地市场和 macOS/Linux Claude。
有其他 CM 任务在运行时先结束或暂停；临时只想检查，可说“cm-check，只检查，不升级”。
...
按描述未复现时，`$cm-fix` 会先按输入、状态、时序、环境、规模逐次改变一个维度主动探索，最多 3 个场景或 15 分钟（先到为准）；命中场景用作红灯测试骨架，到上限仍未复现才进入观测闭环，档案记录全部复现尝试。
...
日常直接运行 `$cm-test`：自动识别主分支，分析当前分支全部已提交差异，
列出受影响业务，并运行项目现有的单测覆盖率检查。没有覆盖率工具就明确标“未测得”。
要一起补测试，说 **“cm-test，并补齐单测”**；也可看完报告后回复 **“补齐单测”**。
只补已授权测试文件，随后重跑并独立审查；不擅自修改产品代码或安装测试框架。
开发需求/修 bug 时，这项检查在最终独立审查前完成。
...
| `all` | 组合 logic、commands、browser；有明确测试目标时的默认模式 | 某一层 PASS 不能覆盖另一层 FAIL |
...
$cm-security
$cm-security --all
$cm-security {项目路径} --semgrep-rules {外部本地规则文件} --osv-db {外部缓存目录}
```

默认检查当前分支相对主分支的差异及已跟踪未提交修改；`--all` 检查全部已跟踪文件。
先读业务地图，再运行现有本地工具并复核鉴权、注入、文件/命令/网络边界。
扫描不自动安装工具、下载规则/数据库、修改代码或发布；工具缺失会继续可做的复核，
