...
逐个看 `scripts/*.test.mjs` 里有没有出现过。写这份文档时的结果是 51 个取值缺覆盖；补掉 A 类的 `auto_fix: never`、`video`、cm-check `configured`、cm-fix `design_change` 和 cm-test 中断步骤恢复之后现在是
**45 个，分布在 42 处**（本次从 47 个、43 处降下）。
...
补完的五条都做了**变异验证**——故意把被测代码改坏，确认测试会红。补测试时这一步不能省：
