...
- 主干已有这些改动，**还没发布**：
  ...
  - 本次补齐 cm-check 可选能力三态测试，并更新这两份清单
- 自动检查：每个 PR 跑 117 份，发版时跑满 124 份。
- 盘点脚本 `scripts/audit-untested-enums.mjs` 报告：**48 个取值缺覆盖，分布在 45 处**。
...
- **盘点脚本只认一种写法**（`['a','b'].includes(x)`），`switch`、对象查表、`Set.has` 都漏掉了。所以「48」是下限。
