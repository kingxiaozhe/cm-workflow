...
本仓库没有常驻服务、不收集遥测，也不保存用户凭证。可选
OpenAI-compatible 适配器只在用户明确启用时从本地环境变量读取 API Key。Codex 安装和可选 Claude
...
- 本项目不读取 `.env`；本地 API Key 只通过进程环境变量提供，其他测试秘密必须放在仓库外的临时环境中。未来若引入 `.env`，必须先加入 `.gitignore`，仅提交无真实值的 `.env.example`。
