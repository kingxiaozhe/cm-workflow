...
**本仓库没有单元测试框架，也不需要引入**——prompt 资产没有可断言的函数返回值。质量门是两道：机器化的 `/cm:check`，和人跑的 dogfood。

## 框架与命令

| 层 | 手段 | 命令 |
| ---- | ---- | ---- |
| 一致性检查（机器） | `/cm:check` | 在 Claude Code 中运行 |
...
**修改 `commands/`、`skills/`、`agents/` 下任何文件后，提交前必须跑一次 `/cm:check`，结论为 PASSED 才提交。**

它覆盖五组，全是历史上真实出过的断链：

1. **角色存在性**——N2/N3 匹配表引用的角色文件存在；反向查孤儿角色（建了没接线）
2. **命名一致性**——frontmatter `name` == 目录名/文件名；无旧前缀残留
3. **命令引用有效性**——`/cm:{命令}`、N1–N8、模式文件、skill 互引全部可达
...
| 发版（升 VERSION） | **同步 `commands/cm:check.md` 的「框架版本基线」**——双写不同步 = 用户被误报"混装" |
