...
- **UI 对表脚本四方配对**：cm-ui-engineer/cm:prd 引用的 `cm-ui-lens-extract.mjs` ↔ `templates/ui-lens/` 源文件 ↔ install.sh 安装行 ↔ auto-update build_pairs（v0.9.41 引入）
...
- **版本一致性（防混装/旧装）**：本命令文件自带基线号 → **框架版本基线: 0.9.42**（发包时与 VERSION 文件同步递增）。比对规则：
  - 基线 = cm-VERSION → 一致，正常
