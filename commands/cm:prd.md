...
2. 基于特征给出 **2-3 套定制方案**，每套包含：技术栈、取舍、**对应的脚手架命令**（如 `npm create vite@latest`、`npx create-next-app`、`taro init`），并标注推荐项及推导理由
3. 人做选择题，不做填空题；确认结果连同脚手架命令写入 ADR，bootstrap T-001 直接使用该命令
