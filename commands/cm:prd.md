...
- **HTML 交互原型（可点击 PRD）**：先从源码提取文本需求；内容为 JS 运行时渲染的 → 用无头浏览器加载后提取文本，并**逐页面/逐交互状态截图**（截图既是需求补充材料，也是 Step 8.5 的候选设计基准）。注意原型通病：通常只画理想态——异常态/空态/边界值靠 Step 5.5 歧义五问补齐
...
- **有 HTML 交互原型**（Step 1 已截图）→ **必须先人工确认**："原型视觉是交互示意，还是要像素还原的设计？"确认还原 → 截图落盘 design-baseline/ 作基准；确认仅示意 → 不作基准，视觉由前端按 design.md 实现（原型工具的视觉通常是示意，照像素还原反而错——此问不可跳过）
...
3. 基于特征给出 **2-3 套定制方案**，每套包含：技术栈、取舍、**对应的脚手架命令**（如 `npx create-expo-app`、`npx create-next-app`、`npm create vite@latest`、`taro init`），并标注推荐项及推导理由（含联网校验的来源）
4. 人做选择题，不做填空题；确认结果连同脚手架命令写入 ADR，bootstrap T-001 直接使用该命令
