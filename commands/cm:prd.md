...
- **HTML 交互原型（可点击 PRD）**：先从源码提取文本需求；内容为 JS 运行时渲染的 → 用无头浏览器加载后提取文本，并**逐页面/逐交互状态截图**（截图既是需求补充材料，也是 Step 8.5 的候选设计基准）。**原型的页面清单与跳转流程必须提取为功能需求 [F-xxx] 和验收标准 [AC-xxx]**——原型首先是需求，其次才是视觉候选；流程进了 AC，无论基准档位怎么选都拦得住流程偏离。注意原型通病：通常只画理想态——异常态/空态/边界值靠 Step 5.5 歧义五问补齐
