...
如果 `$ARGUMENTS` 以 `--change` 开头 → **读取 `~/.claude/commands/cm-prd-modes/change-mode.md`** 执行变更模式（C1–C8）
否则 → 进入新建模式
...
- 确认全新项目 → 标记 `GREENFIELD=true`，**读取 `~/.claude/commands/cm-prd-modes/greenfield.md`** 叠加 G1–G4 规则，Step 4 跳过
...
**二开模式追加规则**（GREENFIELD=false 且本次需求会修改存量代码时生效）→ **读取 `~/.claude/commands/cm-prd-modes/brownfield.md`** 执行 B2 波及面 / B3 防护网基线 / B4 增量 specs / B5 拆分锚定地图。
...
需求涉及**交易/资产/支付/代币/证券/金融营销**时，同时加载 `cm-finance-expert` skill 协同：领域正确性审核 + 营销合规红线扫描 + 合规开放问题（并入 Step 5.5，法域确认结果须写入代码项目 `rules/finance.md` 头部字段——由 /cm:init 或 bootstrap 生成该文件；**该文件不存在时（存量项目中途首次引入金融需求，init 已错过时机）→ 由本步在法域确认后现场补生成**：以 `~/.claude/templates/cm-rules/finance.md` 为骨架生成到代码项目 `.claude/rules/`，写入法域字段，并在 CLAUDE.md 的规则引用列表补 `@rules/finance.md`——法域确认结果必须有标准落盘位，不得散落在 specs 里（实跑教训：dogfood 中法域只能暂记 requirements 非功能需求节）。生成动作属主流程，finance skill 本身仍只举旗不写入）。
...
- Stitch 导出 / HTML 原型 → 用 `~/.claude/templates/cm-ui-lens-extract.mjs` 对基准页提取计算值成表;样式值优先**移植改造**而非重新想象——CSS 里的明文放着不抄、对着渲染截图猜,是流程让 AI 干笨活
