...
项目已有较多存量代码（源码文件 > 30 个）时，提示用户：

> "建议执行 `/codebase-context scan` 通读项目生成 10 份参考文档（业务地图）——之后二开时 /cm:prd 会自动加载它做查重和波及面分析，避免瞎改乱改。现在跑吗？（约几分钟）/ 跳过"

用户同意 → 按 `codebase-context` skill scan 模式执行；跳过 → 继续，后续 /cm:prd 会再提示一次。
