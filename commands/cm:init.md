...
输出格式（三选一）：`📚 业务地图: 已全量生成(源码{N}个) / 已增量刷新(变更{N}个) / 跳过(小项目,源码仅{N}个)`

**判定结果落盘**：把同一行写入 CLAUDE.md 的「业务地图」字段——/cm:prd 据此直接行动，不重复判断、不重复建议（实测教训：25 文件的临界项目，init 说跳过、prd 又建议 scan，两处判断打架）。
