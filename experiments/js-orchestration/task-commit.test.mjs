...
function writeHandoff(file,root,attempt){
  fs.writeFileSync(file,`${JSON.stringify({schema_version:1,task_id:'T-001',attempt,status:'ready_for_review',
    changed_files:['a.js'],implementation_sha256:implementationSha256(root,['a.js']),
    verification:[{command:'node fixture',status:'passed',evidence:'synthetic check'}],
    evidence:['a.js','synthetic check'],blockers:[],scope_deviation:[]},null,2)}\n`);
}
function writeReview(file,handoff,attempt,verdict){
  const blocking=verdict==='approved'?0:1,body=verdict==='approved'?'Zero findings.':'Changes are required.';
...
async function fixture(fn,{attempt=1,bytes=Buffer.from('- [ ] T-001: 中文\r\n'),mode=0o640}={}) {
  ...
  try {
    ...
    for(let current=1;current<=attempt;current++){
      const handoff=path.join(reviewsDir,`login-T-001-a${current}-handoff.json`);
      writeHandoff(handoff,root,current);
      writeReview(path.join(reviewsDir,`login-T-001-r${current}.md`),handoff,current,current===attempt?'approved':'changes_requested');
    }
