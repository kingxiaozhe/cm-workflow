...
`cm-task-gate.mjs`. `--tasks` is explicit so feature-local specs remain the authority. The gate accepts
the backward-compatible `{SPECS_DIR}/tasks.md` layout and the normal
`{SPECS_DIR}/{FEATURE_DIR}/tasks.md` layout (where `{FEATURE_DIR}` may be a numbered
directory such as `2.bulk-clear-completed`), but rejects task files outside the
reviews directory's specs root or its named feature directory. `{FEATURE_SLUG}` is
the review/audit slug without the numeric prefix. Only the review for the same task,
