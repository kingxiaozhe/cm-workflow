...
- Stack: Markdown prompt assets, Bash 3.2-compatible scripts, Python 3 standard-library tooling, PowerShell installers/checks, and Node.js 18+ `.mjs` utilities (Playwright remains optional).
- Framework: Codex plugin and Agent Skills, with Claude Code compatibility surfaces.
- Package manager: none; the repository has no dependency-install step or build artifact.
- Version control: `remote` (`origin`); protect user work and use task-scoped commits.
- Delivery form: local developer-tool distribution for Codex and Claude Code.
- Business map: skipped because the repository has no project manifest or `src/`-style application tree; use `docs/architecture.md` instead.

## Commands

- Dependency install: not applicable.
- Development/build: not applicable; edit Markdown and scripts directly.
- Mechanical consistency: `./scripts/cm-check-runtime.sh`
- Global log fixture: `./scripts/cm-check-runtime.sh --log-fixtures`
- Test fixtures: `bash scripts/test-shell-compat.sh`, `node --test scripts/cm-ai-admission.test.mjs scripts/cm-workflow-config.test.mjs scripts/cm-log-event.test.mjs scripts/cm-task-gate.test.mjs scripts/validate-test-cases.test.mjs`, and `python3 scripts/test-task-gate.py`; Python entrypoints remain compatibility/platform harnesses where documented.
...
- `runtime/`: shared contracts for context, orchestration, review, logging, routing, and task gates.
